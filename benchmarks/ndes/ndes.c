...
void des(immense inp, immense key, int * newkey, int isw, immense * out) {
   ...
   if (*newkey) {
	...
	ANNOT_MAXITER(28); // Provided with the benchmark. - BL
	icd.r = (icd.r <<= 1) | getbit(key,ipc1[j],32);
	icd.l = (icd.l <<= 1) | getbit(key,ipc1[k],32);
      }

      for(i=1;i<=16;i++) {
	ANNOT_MAXITER(16); // Provided with the benchmark. - BL
	pg = kns[i]; ks(/* key,*/ i, &pg); kns[i] = pg;
      }
   }
   itmp.r=itmp.l=0L;
   for (j=32,k=64;j>=1;j--,k--) {
     ANNOT_MAXITER(32); // Provided with the benchmark. - BL
...
void ks(/*immense key, */int n, great * kn) {
   ...
   if (n == 1 || n == 2 || n == 9 || n == 16) {
      ...
      for (i=1;i<=2;i++) {
	ANNOT_MAXITER(2); // Code read  - BL
	icd.r = (icd.r | ((icd.r & 1L) << 28)) >> 1;
	icd.l = (icd.l | ((icd.l & 1L) << 28)) >> 1;
      }
   (*kn).r=(*kn).c=(*kn).l=0;
   for (j=16,k=32,l=48; j>=1; j--,k--,l--) {
     ANNOT_MAXITER(16); // Code read  - BL
